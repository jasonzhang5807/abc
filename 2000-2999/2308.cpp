...
int segpre(int rt,int l,int r,int ql,int qr,int x){
    if(l>qr||r<ql)return -2147483647;
    if(l>=ql&&r<=qr)return v[pre(val[rt],x)];
    int mid=l+r>>1;
    return max(segpre(rt<<1,l,mid,ql,qr,x),segpre(rt<<1|1,mid+1,r,ql,qr,x));
}
int segsucc(int rt,int l,int r,int ql,int qr,int x){
    if(l>qr||r<ql)return 2147483647;
    if(l>=ql&&r<=qr)return v[succ(val[rt],x)];
    int mid=l+r>>1;
    return min(segsucc(rt<<1,l,mid,ql,qr,x),segsucc(rt<<1|1,mid+1,r,ql,qr,x));
}
int main(){
    ...
    for(int i=1;i<=m;++i){
        ...
        if(op==1){
            cin>>l>>r>>k;
            cout<<segqueryrnk(1,1,n,l,r,k)+1<<'\n';
        }
        else if(op==2){
            cin>>l>>r>>k;
            ...
            cin>>l>>r>>k;
            cout<<segpre(1,1,n,l,r,k)<<'\n';
        }
        else{
            cin>>l>>r>>k;
            cout<<segsucc(1,1,n,l,r,k)<<'\n';
        }
    }
    return 0;
}
